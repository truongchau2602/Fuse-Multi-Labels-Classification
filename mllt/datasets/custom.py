...
class CustomDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 see_only=set(),
                 save_info=False,
                 col_name_of_img_path = None,
                 save_folder = None
                 ):
        self.save_folder = save_folder
        self.col_name_of_img_path = col_name_of_img_path
        ...
        if not self.test_mode and not ".csv" in ann_file:
            self._set_group_flag()

        if not self.test_mode and ".csv" in ann_file:
            print("Not test mode and annotation file is csv file. Set group flat!")
            self._set_group_flag_csv()
        # transforms
        self.img_transform = ImageTransform(
            size_divisor=self.size_divisor, **self.img_norm_cfg)
        
        print("Image transformation:", self.img_transform)
        ...
        if class_split is not None:
            ...
            print("mmcv.load(class_split)=", self.class_split)
    ...
    def _set_group_flag_csv(self):
        if 'flag' in self.df.columns:
        # Check if flag column is empty or not
            if self.df['flag'].isna().values.any():
                print("Column 'flag' exists and has empty values.")
                self._set_group_flag_csv_helper()
            else:
                print("\nColumn 'flag' exists and does not have empty values.\n")
                return self.df["flag"].to_numpy()
                
        return self._set_group_flag_csv_helper()

    def _set_group_flag_csv_helper(self):
        """Set flag according to image aspect ratio.

        Images with aspect ratio greater than 1 will be set as group 1,
        otherwise group 0.
        """
        print("custom.py  ----  _set_group_flag_csv()")
        flag_df = self.df

        # Check if column width, heitgh of image exists in csv
        # if not -> create 

        def insert_width_height_column_from_path(df, img_prefix,
                                                img_path_name, 
                                                width_position,
                                                height_position):

            if "width" in df.columns and "height" in df.columns:
                if df["width"].dtype == "int64" and df["height"].dtype == "int64":
                    print("Width and heigth in csv has valid value")
                    return df
            # Tạo các list để lưu trữ chiều rộng và chiều dài của ảnh
            widths = []
            heights = []

            # Lặp lại từng hàng trong dataframe
            for index, row in df.iterrows():
                # Đọc ảnh từ đường dẫn trong cột "img_path"
                img = mmcv.imread(img_prefix+row[img_path_name][1:])
                # Tính toán chiều rộng và chiều dài của ảnh
                height, width, _ = img.shape
                
                # Thêm chiều rộng và chiều dài vào các list tương ứng
                widths.append(width)
                heights.append(height)

            # Thêm cột "width" và "height" vào dataframe
            df.insert(width_position, "width", widths)
            df.insert(height_position, "height", heights)


            return df
        
        flag_df = insert_width_height_column_from_path(flag_df, img_prefix = self.img_prefix,
                                                       img_path_name = self.col_name_of_img_path,
                                                       width_position =1,
                                                       height_position=2)
        ...
        def calculate_flag(row):
            try:
                ...
                if  row["width"] / row["height"] > 1:
                    return 1
                else:
                    return random.randint(0, 1)
            except Exception as e:
                # Xử lý ngoại lệ khi chiều rộng bằng 0
                print(str(e))
                return "Flag error: " + str(e)
                
        # tính toán các giá trị flag mới cho từng hàng
        flag_df['flag'] = flag_df.apply(calculate_flag, axis=1)
        # Lấy cột "flag" ra khỏi dataframe và lưu vào biến flag_col
        flag_col = flag_df.pop("flag")

        # Thêm cột "flag" vào dataframe tại vị trí đầu tiên
        flag_df.insert(0, "flag", flag_col)

        # ghi lại kết quả vào tệp csv
        flag_df.to_csv(self.ann_file, index=False)
        print("Done")
    ...
    def __getitem__(self, idx):
        if self.test_mode:
          print("__getitem__  ------ self.test_mode=", self.test_mode)
