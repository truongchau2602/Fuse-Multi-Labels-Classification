...
class OutOfDistribution(CustomDataset):
    ...
    def __init__(self, **kwargs):
        ...
        print("======= OUT OF DISTRIBUTION TRAINING \n\n")
    ...
    def load_annotations(self, ann_file, LT_ann_file=None):
        ...
        if "imgs" in self.df.columns:
            img_infos = self.df["imgs"].tolist()            
        else:
    ...
    def get_ann_info(self, idx):
        ...
        print(img_name)
        row = self.df.loc[self.df["imgs"]== str(img_name)].iloc[0]

        if "labels" not in self.df.columns:
            label_value = None
            one_hot_vector_label = self._one_hot_encoding(label_value, self.CLASSES)
            ann = dict(labels=one_hot_vector_label)
            return ann
        label_value = row.loc["labels"] # type str
        one_hot_vector_label = self._one_hot_encoding(label_value, self.CLASSES)
        ann = dict(labels=one_hot_vector_label)
        return ann
    def get_index_dic(self, list=False, get_labels=False):
        ...
        save_folder = "mllt/appendix/OODdevkit"
        ...
        for i, img_name in tqdm(enumerate(self.img_infos)):
            ...
            my_df = self.df.loc[self.df["imgs"] == img_name]
            img_id = my_df.iloc[0]["imgs"]
            # print(img_id)
    ...
    def prepare_train_img(self, idx):
        ...
        # Trích xuất từ "aeroplane" từ phần đầu tiên của chuỗi
        class_name = img_parts[1]
        
        # load image
        img = mmcv.imread(osp.join(self.img_prefix, class_name, img_name))
