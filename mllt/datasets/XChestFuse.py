...
class XChestFuse(CustomDataset):
    ...
    def __init__(self, **kwargs):
        super(XChestFuse, self).__init__(**kwargs)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            print("__getitem__  ------ prepare_train_im, test_mode=", self.test_mode)
    ...
    def prepare_train_img(self, idx):
        path_list = self.img_infos[idx]
        print("------- XChestFuse prepare_train_img --------")
        ...
        for img_path in path_list:
            ...
            img_meta_mini = dict(
                ...
                flip=flip)
            img_meta.append(img_meta_mini)
            
            img_list.append(to_tensor(img))
            

        img_torch_tensors = torch.stack(img_list, dim=0)
        # img_torch_tensors= torch.stack(img_list, dim=0).unsqueeze(0)

        # print(type(img_torch_tensors))
        # print(img_torch_tensors.shape)
        # print(type(img_torch_tensors[0]))
        ...
        data = dict(
                img=DC(img_torch_tensors, stack=True),
    ...
    def load_annotations(self, ann_file, LT_ann_file=None):
        ...
        if 'path_list' in self.df.columns:
            import ast
            img_infos = self.df['path_list'].apply(lambda x: ast.literal_eval(x))
            img_infos = img_infos.tolist()
    ...
    def get_ann_info(self, idx):
        img_path = self.img_infos[idx]
        row = self.df.loc[self.df["path_list"]== str(img_path)]
    ...
    def get_index_dic(self, list=False, get_labels=False):
        ...
        for i, img_path in tqdm(enumerate(self.img_infos)):
            ...
            my_df = self.df.loc[self.df["path_list"] == str(img_path)]
