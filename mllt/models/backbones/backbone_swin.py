...
from .backbone_swin_utils_misc import clean_state_dict
...
def build_backbone(args):
    ...
    if args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
        ...
        del backbone.head
        # # Tạo tensor đầu vào
        # input_tensor = torch.randn(1, 3, 384, 384)  # Đặt kích thước phù hợp với backbone

        # # Gọi hàm forward_features() của backbone để tính toán output
        # output = backbone.forward_features(input_tensor)

        # # In thông tin về output
        # print(f"Output backbone shape: {output.shape}")
        ...
        del backbone.head
    elif args.backbone in ["densenet121-res224-all"]:
        backbone = xrv.models.DenseNet(weights="densenet121-res224-all")
        backbone.op_threshs = None # prevent pre-trained model calibration
        #Output backbone shape: torch.Size([1, 1024, 12, 12])
