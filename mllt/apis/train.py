...
def train_classifier(model,
    ...
    elif cfg.dataset_type == "XChestFuse":
        _plain_non_dist_train(classifier=model, dataset=dataset, cfg=cfg, validate=validate, logger=logger)
...
def _plain_non_dist_train(classifier, dataset, cfg, validate=False, logger=None):
    # prepare data loaders
    ...
    data_loaders = [
        build_dataloader(
            ...
            custom_collate = cfg.custom_collate_for_DataLoader)
    ]
    ...
    # criterion
    # criterion = classifier.head.loss(*loss_inputs)
    
    optimizer = build_optimizer(model, cfg.optimizer)
    # Tạo lr_scheduler sử dụng cấu hình lr_config
    # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, **cfg.lr_config)

    # Khởi tạo scheduler
    
    import torch.optim.lr_scheduler as lr_scheduler
    if cfg.lr_config['policy'] == 'step':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=cfg.lr_config['step'], gamma=0.1)
    else:
        raise ValueError('Unsupported learning rate policy: {}'.format(cfg.lr_config['policy']))
...
def _non_dist_train(model, dataset, cfg, validate=False, logger=None):
    ...
    # build runner
    optimizer = build_optimizer(model, cfg.optimizer)
    # print("line 291  mllt/apis/train.py   build_optimizer ends")
    runner = EpochBasedRunner(model, batch_processor, optimizer, cfg.work_dir, logger)
    runner.register_training_hooks(cfg.lr_config, cfg.optimizer_config,
                                   cfg.checkpoint_config, cfg.log_config)
    runner.register_hook(SamplerSeedHook())
    # print("line 296  mllt/apis/train.py   runner.register_hook end")
    ...
    print("\nline 300  mllt/apis/train.py   runner.run begin")
    lines = inspect.getsource(runner.run)
    # for line in lines.split('\n'):
    #     print(line.strip())
    runner.run(data_loaders, cfg.workflow, cfg.total_epochs)
    print("line 305  mllt/apis/train.py   runner.run ends")
